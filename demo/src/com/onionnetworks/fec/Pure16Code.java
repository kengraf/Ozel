        ...
        public class Pure16Code extends PureCode {
            ...
            protected void encode(byte[][] src, int[] srcOff, byte[][] repair,
                ...
                for (int i = 0; i < srcChars.length; i++) {
                    ...
                    Util.arraycopy(src[i], srcOff[i], srcChars[i], 0,
                            packetLength);
                    srcCharsOff[i] = 0;
                }
                ...
                for (int i = 0; i < repair.length; i++) {
                    if (index[i] < k) { // < k, systematic so direct copy.
                        System.arraycopy(src[index[i]], srcOff[index[i]],
                                repair[i], repairOff[i], packetLength);
                    } else {
                        encode(srcChars, srcCharsOff, repairChars, 0, index[i],
                                numChars);
            ...
            protected void decode(byte[][] pkts, int[] pktsOff, int[] index,
                ...
                for (int i = 0; i < pktsChars.length; i++) {
                    ...
                    Util.arraycopy(pkts[i], pktsOff[i], pktsChars[i], 0,
                            packetLength);
                    pktsCharsOff[i] = 0;
                }

                char[][] result = decode(pktsChars, pktsCharsOff, index,
                        numChars);
            ...
            protected char[][] decode(char[][] pkts, int[] pktsOff,
                    int[] index, int numChars) {

                char[] decMatrix = fecMath.createDecodeMatrix(encMatrix, index,
                        k, n);
                ...
                for (int row = 0; row < k; row++) {
                    if (index[row] >= k) {
                        ...
                        for (int col = 0; col < k; col++) {
                            fecMath.addMul(tmpPkts[row], 0, pkts[col],
                                    pktsOff[col], decMatrix[row * k + col],
                                    numChars);
                        }
                    }
                }
