...
import fec_util.Util;
        ...
        public class FECMath {
            ...
            private static Logger logger = Logger.getLogger(FECMath.class
                    .getName());
            ...
            public final void generateGF() {
                ...
                for (i = gfBits + 1; i < gfSize; i++) {
                    if (gf_exp[i - 1] >= mask) {
                        gf_exp[i] = (char) (gf_exp[gfBits] ^ ((gf_exp[i - 1] ^ mask) << 1));
                    } else {
 ...
 *                 // debug
 *
                System.out.println("exp()=" + gfSize );
                for (i = 0; i < gfSize+1; i++) {
                    System.out.print( " " + Integer.toHexString((char)gf_exp[i]) );
                    if( (i%16) ==15 ) {
                    	System.out.println();
                    	}
                }
 ...
 *
                System.out.print("\nlog()=" + gfSize );
                for (i = 0; i < gfSize+1; i++) {
                    System.out.print( " " + Integer.toHexString((char)gf_log[i]) );
                    if( (i%16) ==15 ) {
                    	System.out.println();
                    	}
                }
*/
             ...
             */
            public final void matMul(char[] a, char[] b, char[] c, int n,
                    int k, int m) {
             ...
             * computes C = AB where A is n*k, B is k*m, C is n*m
             */
            public final void matMul(char[] a, int aStart, char[] b,
                    int bStart, char[] c, int cStart, int n, int k, int m) {
            ...
            public final char[] createEncodeMatrix(int k, int n) {
                if (k > gfSize + 1 || n > gfSize + 1 || k > n) {
                    throw new IllegalArgumentException("Invalid parameters n="
                            + n + ",k=" + k + ",gfSize=" + gfSize);
                }
                ...
                for (int pos = k, row = 0; row < n - 1; row++, pos += k) {
                    for (int col = 0; col < k; col++) {
                        tmpMatrix[pos + col] = gf_exp[modnn(row * col)];
                    }
                }
/* TODO DEbug
 *                 // debug
 *
                ...
                for (int row = 0; row < n; row++) {
                	for (int col=0; col<k; col++ ) {
                    System.out.print( " " + Integer.toHexString((char)tmpMatrix[(row*k)+col]));
                	}                   
                	System.out.println();
                }
*/
 ...
 *
                ...
                for (int row = 0; row < n; row++) {
                	for (int col=0; col<k; col++ ) {
                    System.out.print( " " + Integer.toHexString((char)encMatrix[(row*k)+col]));
                	}                   
                	System.out.println();
                }
*/
