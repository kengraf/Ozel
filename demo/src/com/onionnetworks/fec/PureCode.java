        ...
        public class PureCode extends FECCode {
            ...
            protected void encode(byte[][] src, int[] srcOff, byte[][] repair,
                ...
                for (int i = 0; i < repair.length; i++) {
                    encode(src, srcOff, repair[i], repairOff[i], index[i],
                            packetLength);
                }
            }
            ...
            private void encode(byte[][] src, int[] srcOff, byte[] repair,
                ...
                if (index < k) { // < k, systematic so direct copy.
                    System.arraycopy(src[index], srcOff[index], repair,
                            repairOff, packetLength);
            ...
            protected void decode(byte[][] pkts, int[] pktsOff, int[] index,
                    ...
                    if (index[row] >= k) {
                        ...
                        for (int col = 0; col < k; col++) {
                            fecMath.addMul(tmpPkts[row], 0, pkts[col],
                                    pktsOff[col], (byte) decMatrix[row * k
                                            + col], packetLength);
                        }
                    }
                }
                ...
                for (int row = 0; row < k; row++) {
                    if (index[row] >= k) { // only copy those actually decoded.
                        System.arraycopy(tmpPkts[row], 0, pkts[row],
                                pktsOff[row], packetLength);
                        index[row] = row;
                    }
                }
            }
